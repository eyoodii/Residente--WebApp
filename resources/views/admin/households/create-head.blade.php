...
@section('title', 'Register Household Head')
...
<div class="max-w-3xl mx-auto px-4 py-8">
    ...
    <nav class="mb-5">
        <ol class="flex items-center gap-2 text-sm text-gray-500">
            ...
            <li><a href="{{ route('admin.households.show', $household) }}" class="text-sea-green hover:text-deep-forest font-semibold transition-colors">{{ $household->household_number }}</a></li>
            <li><span class="text-gray-300">/</span></li>
            <li class="text-gray-700 font-medium">Add Family</li>
        </ol>
    </nav>

    {{-- Page Header — different copy for first family vs additional --}}
    @php $isFirstFamily = request()->boolean('first') || $household->householdHeads()->count() === 0; @endphp
    <div class="mb-5">
        @if($isFirstFamily)
            ...
            <p class="text-gray-500 text-sm mt-1">Set up the primary family unit for <span class="font-semibold text-deep-forest">{{ $household->household_number }}</span>.</p>
        @else
            <h1 class="text-2xl font-bold text-deep-forest flex items-center gap-2">
                <span class="w-8 h-8 rounded-lg bg-sea-green/10 text-sea-green flex items-center justify-center text-base">👤</span>
                Add Family (HHN)
            </h1>
            <p class="text-gray-500 text-sm mt-1">Register an additional family unit at <span class="font-semibold text-deep-forest">{{ $household->household_number }}</span>.</p>
        @endif
    </div>
    ...
    <div class="mb-5 flex items-center gap-3 bg-sea-green/5 border border-sea-green/20 rounded-lg px-4 py-3">
        ...
        <span class="text-sm text-deep-forest font-medium">{{ $household->full_address }}</span>
    </div>
    ...
    {{-- Alpine.js form with resident / manual toggle --}}
    <div x-data="{
        mode: '{{ old('entry_mode', $availableResidents->isEmpty() ? 'manual' : 'resident') }}',
        hasResidents: {{ $availableResidents->isNotEmpty() ? 'true' : 'false' }}
    }">
        <form method="POST" action="{{ route('admin.households.head.store', $household) }}">
            @csrf
            ...
            <div class="space-y-5">
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-5">
                    <p class="text-sm font-bold text-deep-forest mb-3">How would you like to register the household head?</p>

                    <div class="flex flex-col sm:flex-row gap-3">
                        {{-- Link a registered resident --}}
                        <label :class="[
                                   mode === 'resident' ? 'border-sea-green bg-sea-green/5' : 'border-gray-200 bg-white hover:border-sea-green/40',
                                   !hasResidents ? 'opacity-50 cursor-not-allowed' : ''
                               ]"
                            ...
                            <div>
                                ...
                                <p class="text-xs mt-0.5"
                                   :class="!hasResidents ? 'text-amber-600 font-medium' : 'text-slate-500'">
                                    @if($availableResidents->isNotEmpty())
                                        {{ $availableResidents->count() }} registered resident(s) available.
                                    @else
                                        No registered residents yet — ask them to sign up at
                                        <a href="{{ route('register') }}" target="_blank"
                                           class="underline text-sea-green hover:text-deep-forest">
                        ...
                        <label :class="mode === 'manual' ? 'border-sea-green bg-sea-green/5' : 'border-gray-200 bg-white hover:border-sea-green/40'"
                            ...
                            <div>
                                ...
                                <p class="text-xs text-slate-500 mt-0.5">Head is not yet in the system — enter their name to create the family unit.</p>
                            </div>
                        </label>
                    </div>
                </div>
                ...
                <div x-show="mode === 'resident'" x-transition
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-500 mt-1.5">
                            Only showing residents from <strong>{{ $household->barangay }}</strong> who are not already household heads.
                        </p>
                    </div>
                </div>
                ...
                <div x-show="mode === 'manual'" x-transition
                    ...
                    {{-- Row 1: Last Name + Extension Name (surname used for family linking) --}}
                        ...
                        <div>
                            ...
                            <select name="head_extension_name"
                                ...
                                @foreach(['Jr.', 'Sr.', 'II', 'III', 'IV', 'V'] as $ext)
                                    <option value="{{ $ext }}" {{ old('head_extension_name') == $ext ? 'selected' : '' }}>{{ $ext }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="flex items-start gap-2.5 mt-4 bg-slate-50 border border-slate-200 rounded-lg px-3.5 py-3">
                        ...
                        <p class="text-xs text-slate-500 leading-relaxed">
                            ...
                            You can link a registered resident later from the household detail page.
                        </p>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-end gap-3">
                    ...
                    <button type="submit"
                        ...
                        <span>👤</span> Register Household Head
                    </button>
                </div>
    ...
    <div class="mt-5 bg-sea-green/5 border border-sea-green/20 rounded-lg p-4 flex items-start gap-3">
        ...
        <div>
            <p class="text-sm font-semibold text-deep-forest mb-0.5">Surname Recognition</p>
            <p class="text-sm text-gray-600">
                Once registered, the system will automatically suggest residents with matching surnames at this address for family linking.
            </p>
        </div>
    </div>
