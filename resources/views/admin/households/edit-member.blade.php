...
@section('title', 'Edit Member — ' . $householdHead->household_head_number)
...
<div class="max-w-3xl mx-auto px-4 py-8">
    ...
    <nav class="mb-5">
        <ol class="flex items-center gap-2 text-sm text-gray-500">
            ...
            <li><span class="text-gray-300">/</span></li>
            <li class="text-gray-700 font-medium">Edit Member</li>
        </ol>
    </nav>
    ...
    <div class="mb-5">
        ...
        <p class="text-gray-500 text-sm mt-1">
            Update details for <span class="font-semibold text-deep-forest">{{ $householdMember->full_name }}</span> in the <span class="font-semibold text-deep-forest">{{ $householdHead->head_name }}</span> family.
        </p>
    </div>
    ...
    {{-- Member Context --}}
    <div class="mb-5 flex items-center gap-3 bg-gray-50 border border-gray-200 rounded-lg px-4 py-3">
        <span class="text-sm font-semibold text-gray-600">Member ID:</span>
    ...
    <form method="POST" action="{{ route('admin.households.member.update', [$householdHead, $householdMember]) }}">
        @csrf
        @method('PUT')
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
                    <div>
                        ...
                        <input type="text" name="last_name"
                               value="{{ old('last_name', $householdMember->last_name) }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="first_name"
                               value="{{ old('first_name', $householdMember->first_name) }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="middle_name"
                               value="{{ old('middle_name', $householdMember->middle_name) }}"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="date" name="date_of_birth"
                               value="{{ old('date_of_birth', $householdMember->date_of_birth->format('Y-m-d')) }}"
                               required
                               class="w-full px-3.5 py-2 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-sea-green focus:border-transparent transition
                    ...
                    <div>
                        ...
                        <select name="gender" required
                            ...
                            @foreach(['Male', 'Female', 'Other'] as $g)
                                <option value="{{ $g }}" {{ old('gender', $householdMember->gender) == $g ? 'selected' : '' }}>{{ $g }}</option>
                            @endforeach
                        </select>
                        @error('gender')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <select name="civil_status"
                            ...
                            @foreach(['Single', 'Married', 'Widowed', 'Legally Separated'] as $cs)
                                <option value="{{ $cs }}" {{ old('civil_status', $householdMember->civil_status) == $cs ? 'selected' : '' }}>{{ $cs }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="relationship" required
                            ...
                            @foreach(['Co-Head', 'Spouse', 'Son', 'Daughter', 'Father', 'Mother', 'Brother', 'Sister', 'Grandchild', 'Other Relative'] as $rel)
                                <option value="{{ $rel }}" {{ old('relationship', $householdMember->relationship) == $rel ? 'selected' : '' }}>{{ $rel }}</option>
                            @endforeach
                        </select>
                        @error('relationship')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <input type="text" name="occupation"
                               value="{{ old('occupation', $householdMember->occupation) }}"
