...
@section('title', 'Building Permit Application')
@section('content')
<div class="p-8 max-w-4xl mx-auto space-y-6">
    @include('department.components._module-header', ['icon'=>'🏗️','title'=>'Building Permit #'.$serviceRequest->request_number,'subtitle'=>'Submitted: '.$serviceRequest->requested_at?->format('F d, Y')])
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-4">
            ...
            <dl class="grid grid-cols-2 gap-3 text-sm">
                ...
                <div><dt class="text-xs text-gray-500">Flood Prone</dt><dd>{{ $serviceRequest->resident?->flood_prone ? 'Yes' : 'No' }}</dd></div>
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-3">
            ...
            <form action="{{ route('department.building-permits.approve', $serviceRequest) }}" method="POST">
                @csrf @method('PATCH')
            ...
            <form action="{{ route('department.building-permits.reject', $serviceRequest) }}" method="POST">
                @csrf @method('PATCH')
    ...
    <a href="{{ route('department.building-permits.index') }}" class="inline-flex items-center gap-2 text-sm text-gray-600">← Back to Building Permits</a>
</div>
@endsection
