...
class LoginRequest extends FormRequest
{
    ...
    public function authenticate(): void
    {
        ...
        if ($resident && $resident->isLocked()) {
            $minutes = $resident->locked_until->diffInMinutes(now());
        ...
        if (! Auth::attempt($this->only('email', 'password'), $this->boolean('remember'))) {
            ...
            // Increment failed login attempts for the resident
            if ($resident) {
                $resident->incrementLoginAttempts();
                
                // Log suspicious activity
                $resident->logActivity(
                    'failed_login',
                    "Failed login attempt for {$resident->email}",
                    [
                        'severity' => 'warning',
                        'is_suspicious' => true,
                    ]
